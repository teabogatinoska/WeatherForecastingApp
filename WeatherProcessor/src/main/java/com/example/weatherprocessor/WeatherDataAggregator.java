...
import com.example.weatherprocessor.model.*;
import com.example.weatherprocessor.parser.*;
import com.example.weatherprocessor.processor.WeatherProcessorManager;
...
public class WeatherDataAggregator {
    ...
    public void receiveAirQualityData(String messageJson) {
        System.out.println("Received message for weather-aq-data: " + messageJson);
        try {
    ...
    private void processWeatherData(String topic, String messageJson) {
        try {
            ...
            System.out.println("Weather Data: "+ jsonData);
            // Now deserialize the weatherData string into a JSON object
            Map<String, Object> weatherDataMap = objectMapper.readValue(jsonData, new TypeReference<Map<String, Object>>() {
            });
            ...
            apiCallCount++;
            if (parser != null) {
                WeatherData weatherData = parser.parse(objectMapper.writeValueAsString(weatherDataMap)); // Parse the weatherData
